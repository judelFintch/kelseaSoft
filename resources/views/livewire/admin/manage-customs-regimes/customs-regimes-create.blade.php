<div>
    <div class="max-w-3xl mx-auto py-10 px-6 space-y-8">
        <h2 class="text-xl font-semibold text-gray-800 dark:text-white">Customs Regimes Management</h2>
        ...
        <div>
            <x-forms.input 
                ...
                placeholder="🔍 Search customs regime..." 
                class="w-full"
            />
        </div>
            ...
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700 text-sm">
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                    @forelse($regimes as $regime)
                        <tr>
                            <td class="px-4 py-2 text-gray-900 dark:text-white">{{ $regime->name }}</td>
                            <td class="px-4 py-2 flex gap-3">
                                <button wire:click="edit({{ $regime->id }})" class="text-indigo-600 text-xs hover:underline">Edit</button>
                                @if ($confirmingDelete === $regime->id)
                                    <span class="text-sm text-gray-600 dark:text-gray-300">
                                        Confirm?
                                        <button wire:click="delete({{ $regime->id }})" class="text-red-500 hover:underline ml-1">Yes</button>
                                        <button wire:click="$set('confirmingDelete', null)" class="text-gray-500 hover:underline ml-1">No</button>
                                    </span>
                                @else
                                    <button wire:click="confirmDelete({{ $regime->id }})" class="text-red-500 text-xs hover:underline">Delete</button>
                                @endif
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="2" class="px-4 py-4 text-gray-500 text-center">No customs regimes found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
